...
- Cilentサーバー
    ...
    使用する技術としては、HTML, CSS, JavaScript, Vue3, vuetify, axiosを使用する。<br>
...
## 現状のフロー
- 帳票について<br>
現状のフローとして、月次報告書、交通費、休日届、休日出勤届をすでに配布してあるマクロを含んだエクセルファイルを記述して提出する。<br><br>
- 残有休日数把握について<br>
これまでに記述して月次報告書を見返して、取得した有休日数等を計算する。<br><br>
- 社員追加に関して<br>
...
- パスワード変更<br>
    社員のホーム画面より設定してあるパスワードを変更する機能<br>
...
- 社員一覧画面<br>
    ...
    管理者ユーザーは、閲覧する月を指定するとその月に社員が働いた日数、時間、残業時間等を閲覧することを可能とする画面。<br><br>
- 月次報告書作成画面<br>
...
## データフロー
データフローは以下の8つを実装する。<br>
- 月次報告書データ<br>
    - 月次報告書データ登録<br>
        月次報告書作成画面から、月次報告書テーブルに登録する。<br>

    - 月次報告書データ修正<br>
        月次報告書作成画面から、月次報告書テーブルに登録した情報を修正する。<br>

    - 月次報告書データ削除<br>
        月次報告書作成画面から、月次報告書テーブルに登録したデータを削除する。<br>

- 社員情報<br>
    - 社員情報登録<br>
        社員登録画面から、新たに入社した社員の情報を社員情報テーブルに登録する。<br>

    - 社員情報修正<br>
        社員登録画面から、社員情報テーブルに登録した情報を修正する。<br>

    - 社員情報削除<br>
        社員登録画面から、社員情報テーブルに登録した情報を削除する。<br>

- 社員情報一覧取得<br>
    社員一覧画面起動時に、必要なデータを取得する。<br>

- 月次報告書出力<br>
    月次報告書作成画面から、帳票出力ボタンを押下した時に帳票レイアウトに出力するデータを、月次報告書テーブルから取得する。